...
fn pretend_fields_used_struct(cont: &Container, fields: &[Field]) -> TokenStream {
    ...
    quote! {
        match _serde_core::__private::None::<&#type_ident #ty_generics> {
            _serde_core::__private::Some(#type_ident { #(#members: #placeholders),* }) => {}
            _ => {}
        }
    }
}
...
fn pretend_fields_used_struct_packed(cont: &Container, fields: &[Field]) -> TokenStream {
    ...
    quote! {
        match _serde_core::__private::None::<&#type_ident #ty_generics> {
            _serde_core::__private::Some(__v @ #type_ident { #(#members: _),* }) => {
                #(
                    let _ = _serde_core::__private::ptr::addr_of!(__v.#members);
                )*
            }
            _ => {}
        }
...
fn pretend_fields_used_enum(cont: &Container, variants: &[Variant]) -> TokenStream {
    ...
    quote! {
        match _serde_core::__private::None::<&#type_ident #ty_generics> {
            #(
                _serde_core::__private::Some(#patterns) => {}
            )*
            _ => {}
        }
    }
...
fn pretend_variants_used(cont: &Container) -> TokenStream {
    ...
    let cases = variants.iter().map(|variant| {
        ...
        quote! {
            match _serde_core::__private::None {
                _serde_core::__private::Some((#(#placeholders,)*)) => {
