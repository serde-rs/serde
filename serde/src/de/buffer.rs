...
use crate::{actually_private, de, private, Deserialize, Deserializer, __private::size_hint};

use super::{EnumAccess, Expected, MapAccess, SeqAccess, Visitor};
...
impl<'de> Visitor<'de> for BufferVisitor<'de> {
    ...
    where
        V: SeqAccess<'de>,
    {
        let mut vec = Vec::with_capacity(private::size_hint::cautious(visitor.size_hint()));
    ...
    E: de::Error,
{
        ...
        V: Visitor<'de>,
    {
        match self.content.0 {
            ...
            BufferInner::Some(v) => visitor.visit_some(ContentDeserializer::new(*v)),
            BufferInner::Newtype(v) => visitor.visit_newtype_struct(ContentDeserializer::new(*v)),
    ...
    E: de::Error,
{
    let seq = content
        .iter()
        .map(|buffer| ContentRefDeserializer::new_inner(&buffer.0));
    let mut seq_visitor = de::value::SeqDeserializer::new(seq);
    let value = try!(visitor.visit_seq(&mut seq_visitor));
    try!(seq_visitor.end());
    Ok(value)
    ...
    E: de::Error,
{
    let map = content.iter().map(|(k, v)| {
        (
            ContentRefDeserializer::new_inner(&k.0),
            ContentRefDeserializer::new_inner(&v.0),
        )
    });
    let mut map_visitor = de::value::MapDeserializer::new(map);
    let value = try!(visitor.visit_map(&mut map_visitor));
    ...
    E: de::Error,
{
        ...
        V: Visitor<'de>,
    {
        match *self.content {
            ...
            BufferInner::Unit => visitor.visit_unit(),
            BufferInner::None => visitor.visit_none(),
            BufferInner::Some(ref v) => visitor.visit_some(ContentRefDeserializer::new_inner(&v.0)),
            BufferInner::Newtype(ref v) => {
                visitor.visit_newtype_struct(ContentRefDeserializer::new_inner(&v.0))
            }
...
impl<'a, 'de, E> ContentRefDeserializer<'a, 'de, E> {
    /// Constructs a new [`ContentRefDeserializer`].
    pub fn new(buffer: &'a Buffer<'de>) -> Self {
        Self::new_inner(&buffer.0)
    }

    /// private API, don't use
    pub(crate) fn new_inner(content: &'a BufferInner<'de>) -> Self {
    ...
    E: de::Error,
{
        ...
        V: de::DeserializeSeed<'de>,
    {
        ...
        seed.deserialize(ContentRefDeserializer::new_inner(self.variant))
            .map(|v| (v, visitor))
    }
}
    ...
    E: de::Error,
{
    ...
    fn unit_variant(self) -> Result<(), E> {
        match self.value {
            Some(value) => de::Deserialize::deserialize(ContentRefDeserializer::new_inner(value)),
            None => Ok(()),
        }
    }
    ...
    {
        match self.value {
            Some(value) => seed.deserialize(ContentRefDeserializer::new_inner(value)),
            None => Err(de::Error::invalid_type(
                de::Unexpected::UnitVariant,
                &"newtype variant",
            )),
    ...
    E: de::Error,
{
        ...
        T: de::DeserializeSeed<'de>,
    {
        match self.iter.next() {
            Some(value) => seed
                .deserialize(ContentRefDeserializer::new_inner(&value.0))
                .map(Some),
            None => Ok(None),
        }
    }
    ...
    E: de::Error,
{
        ...
        T: de::DeserializeSeed<'de>,
    {
        match self.iter.next() {
            Some((key, value)) => {
                self.value = Some(value);
                seed.deserialize(ContentRefDeserializer::new_inner(&key.0))
                    .map(Some)
            }
            None => Ok(None),
        }
