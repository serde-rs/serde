...
#[derive(Debug, PartialEq, Deserialize)]
struct DeserializeDuplicate {
    #[serde(deserialize_duplicate = "take_last_duplicate")]
    name: String,
    #[serde(
        deserialize_with = "deserialize_comma_separated",
        deserialize_duplicate = "merge_duplicates"
    )]
    numbers: Vec<u32>,
}

#[test]
fn test_deserialize_duplicate() {
    assert_de_tokens::<DeserializeDuplicate>(
        &DeserializeDuplicate {
            name: "bb".to_string(),
            numbers: vec![1, 90, 91, 11, 28],
        },
        &[
            Token::Struct {
                name: "DeserializeDuplicate",
                len: 5,
            },
            Token::Str("name"),
            Token::Str("aa"),
